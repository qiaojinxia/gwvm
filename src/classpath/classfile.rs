    ...
    pub fn print_constant(&self) {
        ...
        println!(
            ...
            self.constant_pool
                ...
                .unwrap()
                .parse_content(self.constant_pool.as_ref())
                .unwrap()
        );
        println!(
            "[supper_class] <{}> ",
        ...
        for (_, fieldinfo) in self.fields.iter().enumerate() {
            let name = self
                .constant_pool
                .get(fieldinfo.name_index as usize)
                .unwrap()
                .parse_content(self.constant_pool.as_ref())
                .unwrap();
            let descriptor_index = self
                .constant_pool
                .get(fieldinfo.descriptor_index as usize)
                .unwrap()
                .parse_content(self.constant_pool.as_ref())
                .unwrap();
            let access_flags = parse_type(fieldinfo.access_flags);

            println!("\n[Fields↓]");
            println!(
                "Name : {} , Descriptor : {} ,Access flags : {:?}",
                name, descriptor_index, access_flags
            );
            println!(" ---------->  {:?}", fieldinfo.attributes);
        }

        for (_, methodinfo) in self.methods.iter().enumerate() {
            let name = self
                .constant_pool
                .get(methodinfo.name_index as usize)
                .unwrap()
                .parse_content(self.constant_pool.as_ref())
                .unwrap();
            let access_flags = parse_type(methodinfo.access_flags);
            let descriptor_info = self
                .constant_pool
                .get(methodinfo.descriptor_index as usize)
                .unwrap()
                .parse_content(self.constant_pool.as_ref())
                .unwrap();
...
impl Constant {
    pub fn parse_type(&self) -> ConstantType {
        let res = match self {
            Constant::ConstantUtf8Info { constr: _ } => ConstantType::ConstantUtf8Info,
            Constant::ConstantIntegerInfo { i: _ } => ConstantType::ConstantIntegerInfo,
            Constant::ConstantFloatInfo { f: _ } => ConstantType::ConstantFloatInfo,
            Constant::ConstantLongInfo { i: _ } => ConstantType::ConstantLongInfo,
            Constant::ConstantDoubleInfo { f: _ } => ConstantType::ConstantDoubleInfo,
            Constant::ConstantClassInfo { name_index: _ } => ConstantType::ConstantClassInfo,
            Constant::ConstantStringInfo { string_index: _ } => ConstantType::ConstantStringInfo,
            Constant::ConstantFieldrefInfo {
                class_index: _,
                name_and_type_index: _,
            } => ConstantType::ConstantFieldrefInfo,
            Constant::ConstantMethodrefInfo {
                class_index: _,
                name_and_type_index: _,
            } => ConstantType::ConstantMethodrefInfo,
            Constant::ConstantInterfaceMethodrefInfo {
                class_index: _,
                name_and_type_index: _,
            } => ConstantType::ConstantInterfaceMethodrefInfo,
            Constant::ConstantNameAndTypeInfo {
                name_index: _,
                descriptor_index: _,
            } => ConstantType::ConstantNameAndTypeInfo,
            Constant::ConstantMethodHandleInfo {
                reference_kind: _,
                reference_index: _,
            } => ConstantType::ConstantMethodHandleInfo,
            Constant::ConstantMethodTypeInfo {
                descriptor_index: _,
            } => ConstantType::ConstantMethodTypeInfo,
            Constant::ConstantInvokeDynamicInfo {
                bootstrap_method_attr_index: _,
                name_and_type_index: _,
            } => ConstantType::ConstantInvokeDynamicInfo,
    ...
    */
    pub fn parse_content(&self, vec: &Vec<Constant>) -> Option<String> {
        let res = match self {
            Constant::ConstantUtf8Info { constr } => Some(constr.to_string()),
            Constant::ConstantIntegerInfo { i } => Some(i.to_string()),
            Constant::ConstantFloatInfo { f } => Some(f.to_string()),
            Constant::ConstantLongInfo { i } => Some(i.to_string()),
            Constant::ConstantDoubleInfo { f } => Some(f.to_string()),
